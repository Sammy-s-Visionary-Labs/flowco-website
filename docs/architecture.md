...
The owner-confirmed production phone, phone link, public email, and inherited lead recipient are `(419) 709-5808`, `tel:+14197095808`, and `Ohioflowcollc@gmail.com`. `contactDataStatus` marks this contact configuration as production-ready. Public form exposure still requires the separate abuse-protection and production re-verification gates recorded below.
...
- The Resend adapter is implemented. It sends from `requests@notifications.ohioflowco.com` to the canonical production recipient, and its server-only API key is stored in Vercel. The local environment has no key, so local submissions still fail closed by design.
...
- The static sender is `requests@notifications.ohioflowco.com` on the owner-verified `notifications.ohioflowco.com` domain. The canonical recipient is `Ohioflowcollc@gmail.com`; submitted values cannot control sender, recipient, subject, or reply headers.
...
- `contactDataStatus.productionReady` is now `true` because the owner supplied production phone and email values. `requestServiceDeliveryStatus.productionReady` remains `false`, so Vercel production submissions still fail closed until public abuse protection and production delivery re-verification are complete.
...
- Customer email remains optional and no customer autoresponder is authorized.
...
| B-004 | The previously recorded public phone number was `(567) 358-1055`. | Superseded by B-029 | Phase 0.2 |
| B-005 | The previously recorded public email was `Ohioflowcollc@gmail.com`. | Confirmed by B-029 with canonical capitalization | Phase 1.1 brief |
